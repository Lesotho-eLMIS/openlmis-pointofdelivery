...
/**
 * Spring oriented utility class to load data into a database. When given
 * Spring's
 * {@link JdbcTemplate}, an instance of this class is able to run SQL
 * inserts/updates against the
 * attached datasource. It knows what SQL to run, or what data to load, based on
 * Spring
...
public class Resource2Db {
  ...
  /**
   * Update the database from a Resource which has lines of SQL. One SQL statement
   * per line.
   * 
   ...
   * @param resource  the Resource as a CSV, with a header, that has the data to
   *                  load.
   * @throws IOException              if the Resource can't be used.
  ...
  /*
   * converts a Resource into a List of Strings - used when those strings are
   * direct SQL
   */
  ...
  /*
   * converts a Resource which is a CSV, into a Pair where Pair.left is the SQL
   * column names,
   * and Pair.right is the rows of data which go into those columns (each row is
   * an array, the array
  ...
  /*
   * runs the list of SQL strings directly on the database - could be insert /
   * update
   */
  ...
  /**
   * Inserts data into a single table. Given the columns and a list of data to
   * insert, will
   * run a batch update to insert it.
   * 
   * @param tableName      the name of the table (including schema) to insert
   *                       into.
   * @param dataWithHeader a pair where pair.left is an ordered list of column
   *                       names and pair.right
   *                       is an array of rows to insert, where each row is
   *                       similarly ordered as

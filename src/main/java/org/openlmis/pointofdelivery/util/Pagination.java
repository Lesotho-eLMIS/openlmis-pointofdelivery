...
public final class Pagination {
  ...
  /*
   * Because Spring itself uses 0 as the default pageNumber, this value probably
  ...
  /**
   * Returns the Page for a subset of the specified list, determined by the
   * pageable passed in.
   * 
   * @param originalList A list of values, some or all of which should be included
   *                     in a page.
   * @param pageable     An object used to encapsulate the pagination related
   *                     values: page and size.
   */
  public static <T> Page<T> getPage(List<T> originalList, Pageable pageable) {
    ...
    // Validate toIndex.
    // Note that toIndex is exclusive, which is why we don't want
    // originalList.size() - 1.
  ...
  /**
   * Returns the Page for the entirety of the specified list. Intended for use
   * when the
   * supplied list already contains exactly the set of elements requested and
   * there's
